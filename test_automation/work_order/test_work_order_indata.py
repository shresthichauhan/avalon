...
def test_work_order_data_datahash_null(setup_config):
    ...
    check_result = '''{"result": {"code": -8,"message": "Hex digit is not valid"}}'''
   ...
   # process worker actions
    ...
    if err_cd == 0:
        ...
        has received expected result for passing a null  in data and dataHash and the message is %s. \n''', check_get_result["result"]["message"])
    else:
...
def test_work_order_datahash_random_str(setup_config):
    ...
    if err_cd == 0:
        logger.info('''Test Case Success : WorkOrderGetResult response
        has received expected result for passing a random string in dataHash and the message is %s. \n''', check_get_result["result"]["message"])
    else:
        logger.info('''Test Case Failed : WorkOrderGetResult response
        did not contain expected result. ''')
...
def test_work_order_encryptedDataEncryptionKey_str(setup_config):
    """
       Testing work order request by passing a string in encryptedDataEncryptionKey.
    """
    ...
    # Passing input and output file names
    input_json_file = './work_order/input/work_order_encryptedDataEncryptionKey_str.json'
    input_type = 'file'
    output_json_file_name = 'work_order_encryptedDataEncryptionKey_str'
    tamper = {"params": {}}

    # Checking expected responses for the work order submit and work order get result
    check_submit = {"error": {"code": 5}}
    check_get_result = {"result": {"code": -8}}
    ...
    if err_cd == 0:
        logger.info('''Test Case Success : WorkOrderGetResult response
        has received expected result for passing a string in encryptedDataEncryptionkey is %s. \n''', check_get_result["result"]["code"])
    else:
        logger.info('''Test Case Failed : WorkOrderGetResult response
        did not contain expected result. ''')
